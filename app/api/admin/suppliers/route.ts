...
import { adminErrorResponse, createSupplier, type AdminSupplier } from "@/lib/adminDb";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body.name?.trim()) return NextResponse.json({ ok: false, message: "공급사명은 필수입니다." }, { status: 400 });
